...
class Rule:
    def __init__(self, match_expr: Expr, rewrite_expr: Expr, cond: str, let_list: List[Tuple[str, str]], line: int):
        self.match_expr: Expr = match_expr
        self.rewrite_expr: Expr = rewrite_expr
        self.cond: str = cond
        self.let_list: List[Tuple[str, str]] = let_list
        self.line: int = line
...
class RulesParser:
    ...
    def parse_rule(self) -> Rule:
        ...
        let_list: List[Tuple[str, str]] = []
        while self.try_consume('KEYWORD_LET'):
            var_name = self.expect_token('IDENT').value
            var_init = self.expect_token('VERBATIM').value
            let_list.append((var_name, var_init))
