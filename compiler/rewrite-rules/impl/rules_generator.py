...
public:
  explicit GeneratedPass(rewrite_rules::Context &ctx) : ctx_{ctx} {}
...
class RulesGenerator:
    ...
    def __print_rule(self, rule: Rule, printer: CodePrinter):
        ...
        for v in rule.let_list:
            var_name, var_init = v
            printer.write_line(f"auto {var_name} = {var_init};")
        ...
        if can_rewrite_inplace:
            ...
            printer.write_line("v_->set_children(0, " + ', '.join(args) + ");")
            ...
            printer.write_line(f"auto replacement_ = {replacement};")
    ...
    def print_matcher_cond(self, e: Expr, name: str, printer: CodePrinter, vars_declared: Set[str]):
        ...
        if vertex_info.is_variadic():
            ...
            if e.dot3pos is None or range_min_index > e.dot3pos:
                ...
                printer.write_line(f"if ({name}->{range_info.name}().size() != {expected_len}) {{ break; }}")
                ...
                printer.write_line(f"if ({name}->{range_info.name}().size() < {expected_len}) {{ break; }}")
        if e.vertex_string:
            printer.write_line(f"if ({name}->get_string() != std::string_view({e.vertex_string})) {{ break; }}")
        if named_range:
            printer.write_line(f"const auto &{named_range[0]} = {name}->{named_range[1]}();")
            return
        for i, subexpr in enumerate(e.members):
            arg_info, range_offset = vertex_info.get_arg(i, len(e.members))
            if arg_info.optional:
                printer.write_line(f"if (!{name}->has_{arg_info.name}()) {{ break; }}")
            arg_expr = f"{name}->{arg_info.name}()" if range_offset is None else f"{name}->{arg_info.name}()[{range_offset}]"
            ...
            if subexpr.name and subexpr.name in vars_declared:
                printer.write_line(f"if (!is_same({subexpr.name}, {arg_expr})) {{ break; }}")
            else:
                if subexpr.op == Expr.OP_ANY:
                    if new_name != "_":
                        printer.write_line(f"const auto &{new_name} = {arg_expr};")
                else:
                    printer.write_line(f"const auto &{new_name} = vertex_cast<{subexpr.op}>({arg_expr});")
                    printer.write_line(f"if (!{new_name}) {{ break; }}")
