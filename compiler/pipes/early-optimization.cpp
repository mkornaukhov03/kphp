...
class EarlyOptimization final : public FunctionPassBase {
public:
...
private:
  ...
  VertexPtr insert_tmp_string(VertexAdaptor<op_func_call> root) {
    // for every f(substr(...)) replace substr(...) with tmp_substr(...)
    ...
    if (!substr_arg || !substr_arg->func_id->has_tmp_string_specialization()) {
      return root;
    }
    substr_arg->str_val = "_tmp_" + substr_arg->func_id->name;
